...
window.onload = async () => {
    getId("btnLogout").addEventListener('click', handleLogout);
    ...
    let aux2 = await httpRequest.get(('recommendation/history/' + user.steam_id));
    setHistory(aux2.data);
    getId("btnEditUsr").addEventListener('click', () => {
        ...
        getId("btnSaveEdit").addEventListener('click', async () => {
            ...
            if (username != user.nickname || toMD5(password) != user.password) {
                formAlert("modalAlert", "Nome de usuário ou senha Incorretos", "#ff0000", 8, 3000);
                return;
    ...
    getId("btnDelUsr").addEventListener('click', async () => {
        ...
        getId("btnExeDel").addEventListener('click', async () => {
            ...
            if (username != user.nickname || toMD5(password) != user.password) {
                formAlert("modalDelAlert", "Nome de usuário ou senha Incorretos", "#ff0000", 8, 3000);
                return;
            } else {
...
function handleDelete(res) {
    if (res.status === "DELETED") {
        ...
        formAlert("modalDelAlert", "Ocorreu um erro ao excluir sua conta, tente novamente", "#ff0000", 8, 5000);
    }   
}
...
function handleUpdate(res, nickname, password) {
    if (res.status === "UPDATED") {
        ...
        formAlert("modalAlert", "Ocorreu um erro ao editar o usuário, tente novamente", "#ff0000", 8, 5000);
    }
} 
...
async function setHistory(data) {
    let html = `<h2>Histórico de recomendações</h2>`;
    for(let i = 0; i < data.length; i++) {
        html += `<span>${data[i]}</span><br>`;
    }
    getId('recoHistory').innerHTML = html;
}
