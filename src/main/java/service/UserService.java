...
public class UserService {
	...
	public static String checkNickname(Request req, Response res) {
		if (userDao.checkNickname(req.params(":nickname"))) {
			res.status(200);
			res.body("{\"status\":true}");
		} else {
			res.status(200);
	...
	public static void checkAuth(String auth, String id) {
		if (!userDao.checkAuth_key(auth, id))
			halt(401);
	}
	
	public static String update(Request req, Response res) {
		User u = new Gson().fromJson(req.body(), User.class);
		checkAuth(req.headers("Authorization"), u.getSteam_id());
		if (userDao.update(u)) {
			res.status(200);
	...
	public static String delete(Request req, Response res) {
		checkAuth(req.headers("Authorization"), req.params(":steam_id"));
